...
export const getRelatedProjects = (slug: string) => {
  ...
  const currentTechSet = new Set(
    currentProject.technologies.map((t) => t.toLowerCase()),
  );
  ...
  return projectsWithSharedTech.slice(0, 2).map((item) => item.project);
};
...
export const getAllProjects = () => {
  ...
  const publishedProjects = projects.filter(
    (pro, idx) => pro.isPublished === true && isPublishedWithDate(pro),
  );

  if (publishedProjects.length === 0) return [];

  const publishedProjectsSortedByPublishedDates = publishedProjects.sort(
    (a, b) =>
...
function isPublishedWithDate(
  ...
  return (
    project.isPublished === true && typeof project.publishedOn === "string"
  );
}
